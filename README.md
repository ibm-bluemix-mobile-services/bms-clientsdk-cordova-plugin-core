...
The Android development environment does not require additional configuration. Use the Cordova CLI to build the project:

```Bash
cordoba build android
```

Once you build your project through the command-line, you can open the Android Project generated by Cordova in the `[your-app-name]/platforms/android` directory with Android Studio.

## Usage

### BMSClient

The `BMSClient` class is your entry point to the Bluemix Mobile Services SDK. Initializing the `BMSClient` instance is required before sending requests.
